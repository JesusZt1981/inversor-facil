...
(()=>{
  ...
  if(signup&&!qs('#forgotPasswordBtn')){
    ...
    b.onclick=async()=>{
      ...
      try{
        const d=await api('/api/auth/forgot',{method:'POST',body:JSON.stringify({email})});
        toast(d.message||'Revisa tu correo para cambiar la contraseña.');
  ...
  btn.onclick=async()=>{
    ...
    try{
      await api('/api/auth/reset-password',{method:'POST',body:JSON.stringify({access_token:hash.get('access_token'),password})});
      history.replaceState(null,'',location.pathname+location.search.replace(/([?&])recovery=1(&|$)/,'$1').replace(/[?&]$/,''));
      overlay.remove();
