...
async function syncStatement(client,card,end){
  ...
  if(!old){
    const {data,error}=await client.from('card_statements').insert({user_id:card.user_id,card_id:card.id,period_start:start,period_end:periodEnd,due_date:due,calculated_amount:calculated,status:calculated<=0?'paid':'pending'}).select('*').single();
    if(error)throw error;
    return data;
  }
  const finalTarget=old.statement_amount==null?calculated:Number(old.statement_amount);
  const paid=Number(old.paid_amount||0);
  const {data,error}=await client.from('card_statements').update({period_start:start,due_date:due,calculated_amount:calculated,status:paid+.005>=finalTarget?'paid':'pending',updated_at:new Date().toISOString()}).eq('id',old.id).select('*').single();
  if(error)throw error;
  return data;
}
...
async function sendMail(to,subject,htmlContent){
  ...
  if(!r.ok)throw new Error(`Brevo ${r.status}: ${text.slice(0,300)}`);
}

async function probeBrevo(){
  const r=await fetch('https://api.brevo.com/v3/account',{headers:{accept:'application/json','api-key':BREVO_API_KEY}});
  if(!r.ok)throw new Error(`Brevo API key rechazada (${r.status})`);
  return true;
}
...
async function runReminders({probe=false}={}){
  ...
  const {data:cards,error:cardsError}=await client.from('credit_cards').select('*').eq('is_active',true);
  ...
  const {data:settings,error:settingsError}=await client.from('user_settings').select('user_id,notification_email,email_notifications').eq('email_notifications',true);
  ...
  const {data:statements,error:stError}=await client.from('card_statements').select('*').eq('status','pending');
  if(stError)throw stError;

  let sent=0,skipped=0,failed=0;
  const errors=[];
  for(const s of statements||[]){
    const to=emails.get(s.user_id),card=cardMap.get(s.card_id);
    if(!to||!card||remaining(s)<=0){skipped++;continue}
    const delta=daysBetween(today,s.due_date);
    let kind=null;
    if(s.period_end===today)kind='cutoff';
    else if(delta===5)kind='due_5';
    else if(delta===1)kind='due_1';
    else if(delta===0)kind='due_today';
    if(!kind){skipped++;continue}
    const {data:already}=await client.from('notification_log').select('id').eq('statement_id',s.id).eq('kind',kind).maybeSingle();
    if(already){skipped++;continue}
    try{
      const m=mailFor(kind,s,card);
      await sendMail(to,m.subject,m.htmlContent);
      const {error:logError}=await client.from('notification_log').insert({user_id:s.user_id,statement_id:s.id,kind,sent_to:to});
      if(logError && logError.code!=='23505')throw logError;
      sent++;
    }catch(e){failed++;errors.push({statement_id:s.id,error:e.message})}
  }
  return {ok:failed===0,today,cards:(cards||[]).length,statements:(statements||[]).length,sent,skipped,failed,probe:probe?'brevo-ok':'not-requested',errors:errors.slice(0,10)};
}
